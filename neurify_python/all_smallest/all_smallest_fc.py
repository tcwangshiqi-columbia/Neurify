...
for key in list(results_dict.keys()):
    ...
    for smallest_expl in all_of_len:
        ...
        res = 1
        try:
            input_bounds = [[flat_x[i]-eps if flat_x[i]-eps >= 0 else 0, flat_x[i]+eps if flat_x[i]+eps <= 1 else 1] for i in range(len(flat_x))]
            u_bounds = [e[1] for e in input_bounds]
            l_bounds = [e[0] for e in input_bounds]
            res = entails(hit_set,len(hit_set),flat_x,len(flat_x),u_bounds,l_bounds,net_path)
        except:
            pass
        if res == 0:
    ...
    out_string += '=====================================================================\n'
    print(out_string)
    output_file = 'results/all_smallest_{}.txt'.format(config['keras_file'].split('/')[-1][:-3])
