...
bool check_not_max_norm1(struct NNet *nnet, struct Matrix *output){
    float t = output->data[nnet->target];
    for(int i=0;i<nnet->outputSize;i++){
        if(output->data[i]>t && i != nnet->target){
            return true;
        }
    }
    return false;
...
bool check_functions1(struct NNet *nnet, struct Matrix *output){
    ...
    return check_not_max_norm1(nnet, output);
}
